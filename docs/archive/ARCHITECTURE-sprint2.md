...
- Future (Sprint 3): when Claude Gateway/Claude MCP work begins, it brings its own MCP server — distinct in kind from `internal-mcp-server` and `ollama-mcp-server`, which both expose linear/deterministic tools. The Claude-facing MCP is for non-deterministic tool use (the discovery/wizard and deep-reasoning roles described in OVERVIEW.md's AI Model Roles). Flagged by Jacob 2026-09-08 so Sprint 3 planning doesn't lose the distinction — not a Sprint 2 decision.
